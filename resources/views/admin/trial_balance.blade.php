        ...
        <table id="example" class="table table-striped table-bordered display nowrap" width="100%">
          ...
          @foreach($customer as $results)
            <tr>
              ...
              <td>{{number_format($results->current_balance)}}</td>
              <td>0</td>
              @else
              <td>0</td>
               <td>{{number_format(-$results->current_balance)}}</td>
        ...
        @if($expense>0)
          ...
          <!--@endif-->
              ...
              <tr>
                  ...
                  <td><h3>Total</h3></td>
                  <td><h5>PKR {{number_format(($total_amount2)+($expense_inc)+($sale_decrease)+($increase)+($exp_decrease)+($lib_inc)+
                    
                    +($detail_purchase_sum_return)+($Capital_sub_dec)+($purchase_inc)+ (-$current_balance2_purchase))}}
                   
                    </h5></td>
                    
                  <td><h5>PKR {{number_format(($decrease)+($total_amount)+($detail_purchase_sum_return)+($lib_decrease)+
