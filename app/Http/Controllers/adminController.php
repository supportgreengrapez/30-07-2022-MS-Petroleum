...
class adminController extends Controller
{
    ...
    public function trial_balance()
    {
        ...
        $Capital = DB::select("select* from account where nature_of_account = 'Capital'");
        if (count($Capital) > 0)
        {
        ...
        $capital_increase = DB::table('account')->where('nature_of_account', 'Capital')
            ->sum('increase');
        
        $capital_decrease = DB::table('account')->where('nature_of_account', 'Capital')
            ->sum('decrease');

        $Capital_sub = DB::select("select * from account where  nature_of_account='Capital' LIMIT 1");
        // return $Capital_sub;
        ...
        $Capital_sub_decrease = DB::select("select * from account where nature_of_account ='Capital' and decrease != 'NULL'  ");
        ...
        $result = DB::select("select* from account where nature_of_account = 'Capital' ");
        if (count($result) > 0)
        {
        ...
        $cash = DB::select("select* from account where account_type = 'Cash' ");
        if (count($cash) > 0)
        {
            $cashh = $cash[0]->increase;
            $cashh_dec = $cash[0]->decrease;
        }
        else
        {
            $cashh = '0';
...
quantity,rate,amount) values (?,?,?,?,?,?,?,?)", array(
        ...
        if($payment_type =="Payment"){
            ...
            if (count($account_cash))
            {
                ...
                if($account_cash2 >= $test_amount){
                    ...
                    $account_cash3 = DB::select("select* from account where account_name = 'Account Receivable'");
                    $account_cash4 = $account_cash3[0]->balance;
                    $partial2 = $account_cash4 + $test_amount;
                    $account_Receivable ="Account Receivable";
                    $account_cash = DB::table('account')->where('account_name', $account_name)->update(['balance' => "$partial",'increase' => "$partial" ]);
                    $account_cash = DB::table('account')->where('account_name', $account_Receivable)->update(['balance' => "$partial2",'increase' => "$partial2"]);
                    // $increase = DB::table('account')->where('account_name', $account_name)->update(['increase' => "$partial2"]);
                }
                else{
                    return Redirect::back()->withErrors('Amount in payment account is insufficient...');
    ...
    public function account_supplier_form(Request $request)
    {
        ...
        if($payment_type =="Received"){
            ...
            if (count($account_cash))
            {
                ...
                $account_cash3 = DB::select("select* from account where account_name = 'Account Payable'");
                $account_cash4 = $account_cash3[0]->balance;
                $partial2 = $account_cash4 - $test_amount;
                $account_Payable ="Account Payable";
                $account_cash = DB::table('account')->where('account_name', $account_name)->update(['balance' => "$partial",'increase' => "$partial" ]);
                $account_cash = DB::table('account')->where('account_name', $account_Payable)->update(['balance' => "$partial2",'increase' => "$partial2" ]);
                // $increase = DB::table('account')->where('account_name', $account_name)->update(['increase' => "$partial2"]);
            }
            else
            {
    ...
    public function zero()
    {
        ...
        DB::delete("delete from purchase_invoice");
        DB::delete("delete from sale_invoice");
